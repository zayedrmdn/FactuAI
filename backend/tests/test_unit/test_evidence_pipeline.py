...
def test_scrape_article_returns_text():
    ...
    with patch('extract.scraper.requests.get') as mock_get:
...
def test_rank_sentences_returns_scored_tuples():
    ...
    with patch('services.ranking.scorer.get_embed_model') as mock_get_model:
        # Mock the model to return None (triggers fallback)
        mock_get_model.return_value = None
        
        ranked = rank_sentences(claim, sentences)
...
@patch('search.google.search_google')
@patch('search.newsapi.search_newsapi')
@patch('extract.scraper.scrape_article')
@patch('extract.base.extract_sentences')
@patch('services.ranking.scorer.rank_sentences')
def test_collect_evidence_integration(mock_rank, mock_extract, mock_scrape, mock_news, mock_google):
    ...
    with patch.dict('search.base.PROVIDER_FUNCTIONS', {
        'google': mock_google,
        'newsapi': mock_news,
        'tavily': MagicMock()
    }):
...
def test_search_google_requires_api_key():
    ...
    with patch('search.google.GOOGLE_API_KEY', None):
        with patch('search.google.GOOGLE_CX_ID', 'test-id'):
            try:
...
def test_search_newsapi_requires_api_key():
    ...
    with patch('search.newsapi.NEWS_API_KEY', None):
